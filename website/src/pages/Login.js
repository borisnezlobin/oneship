...
const LoginPage = () => {
  ...
  return (
    <div className="m-0 flex h-full flex-col items-center justify-center md:ml-64">
      ...
      <hr />
      <div className="h-8" />
      <div className="w-256 flex flex-col items-center justify-center gap-2">
        <p className="text-theme text-center font-bold">Or</p>
        <button
          className="btn w-256"
          onClick={() => {
            setIsNavigating(true);
            nav(
              "/register?continue=" +
                encodeURIComponent(next && next !== "null" ? next : "/feed"),
            );
          }}
        >
          Create an account
