...
function App() {
  ...
  const canDownloadAndroidApp = window.navigator.userAgent
    .toLowerCase()
    .includes("android");
    ...
    <DataContext.Provider value={{ data: startupData }}>
      <UserDataContext.Provider value={{ userData, setUserData }}>
        ...
        {canDownloadAndroidApp && (
          <div
            className="fixed top-0 w-full bg-gray-800 py-2 text-center text-white"
            onClick={() =>
              window.open("https://oneship.vercel.app/internal/PalyOneShip.apk")
            }
          >
            <p>Download the OneShip app!</p>
            <p>The file is not dangerous</p>
          </div>
        )}
        <Navbar />
