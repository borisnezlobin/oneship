  ...
  useEffect(() => {
    const getStartupData = async () => {
      ...
      try {
        ...
        try {
          ...
          if (schedule == null || schedule.date != today) {
            if (data.schedule != null) {
              ...
              setNotificationForClasses(
                data.schedule,
                userData && userData.data ? userData.data.classNotification : 5,
              );
        ...
        logError(
          "couldn't fetch startup data from " +
            CONFIG.serverURL +
            "api/startup",
        );
      }
    ...
    const getUserData = async () => {
      ...
      if (user_data != null && userData == null)
        try {
        ...
        <View style={tailwind("w-full h-full bg-white")}>
          <ScheduleContext.Provider value={{ schedule, setSchedule }}>
            <UserDataContext.Provider
              value={{ userData, setUserData, setUserDataAndNotify }}
            >
      ...
      <Tabs.Screen
        ...
        options={{
          tabBarIcon: ({ focused, color, size }) =>
            focused ? (
              <ClockIcon color={CONFIG.bg} />
            ) : (
              <ClockOutline color={CONFIG.text} />
            ),
        }}
      />
      <Tabs.Screen
        ...
        options={{
          tabBarIcon: ({ focused, color, size }) =>
            focused ? (
              <TrophyIcon color={CONFIG.bg} />
            ) : (
              <TrophyOutline color={CONFIG.text} />
            ),
        }}
      />
      <Tabs.Screen
        ...
        options={{
          tabBarIcon: ({ focused, color, size }) =>
            focused ? (
              <HomeIcon color={CONFIG.bg} />
            ) : (
              <HomeOutline color={CONFIG.text} />
            ),
        }}
      />
      <Tabs.Screen
        ...
        options={{
          tabBarIcon: ({ focused, color, size }) =>
            focused ? (
              <NewspaperIcon color={CONFIG.bg} />
            ) : (
              <NewspaperOuline color={CONFIG.text} />
            ),
        }}
      />
      <Tabs.Screen
        ...
        options={{
          tabBarIcon: ({ focused, color, size }) =>
            focused ? (
              <Cog6ToothIcon color={CONFIG.bg} />
            ) : (
              <CogOutline color={CONFIG.text} />
            ),
        }}
      />
    </Tabs.Navigator>
