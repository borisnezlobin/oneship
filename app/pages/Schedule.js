...
const SchedulePage = ({ navigation }) => {
  ...
  var height =
    Dimensions.get("window").height - 56 - useSafeAreaInsets().bottom;

  if (schedule == null || calendar == null) {
    return (
      <SafeAreaView
        style={tailwind(
          "bg-white w-full h-full flex justify-center items-center",
        )}
      >
  ...
  if (schedule.value != null) {
    ...
    var show0Period =
      schedule.value[0].name == "0 Period" &&
      userData &&
      userData.data &&
      userData.data.show0;
    ...
    scheduleComponent = !useFixedHeight ? (
      <View>
        {schedule.value.map((period, index) => {
          if (userData && !userData.data.show0 && period.name == "0 Period")
            return null;
          var temp = prevStart;
            ...
            <>
              {hasPassing && (
                ...
                >
                  ...
                  >
                    {period.start - prevPeriod.end}m Passing
                  </Text>
                  <View
                    style={{
                      height: 1,
      ...
      >
        ...
        (eventsToday.length == 1 &&
          ...
          >
            ...
            />
            <Text
              style={[tailwind("text-2xl font-bold"), { color: CONFIG.green }]}
            >
              No Events Today
            </Text>
          </View>
