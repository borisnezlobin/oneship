...
const SettingsPage = () => {
  ...
  const { userData, setUserData, setUserDataAndNotify } =
    ...
    if (userData && editedSettings) {
      ...
      for (var key in editedSettings) {
        if (
          ...
          editedSettings[key] != userData.data[key]
        ) {
          change = true;
          break;
        }
  ...
  const saveUserData = () => {
    ...
    fetch(CONFIG.serverURL + "/api/user/settings", {
      ...
      .then(async (res) => {
        ...
        if (res.status != 200) {
          logError(
            "Error updating user data: server responded with " +
              res.status +
              ", " +
              text,
          );
      ...
      .catch((err) => {
        Toast.show("Error communicating with the server.", {
          ...ERROR_TOAST,
          containerStyle: {
            top: insets.top,
          },
  ...
  const settingsComponent = (
    ...
    <SafeAreaView style={tailwind("bg-white w-full h-full")} key={userData}>
      <ScrollView style={[tailwind("bg-white w-full p-4")]}>
        <Text
          style={[
            tailwind("text-4xl w-full font-bold text-center"),
            { color: CONFIG.green },
          ]}
        >
        ...
        <Text
          style={[
            tailwind("text-lg w-full text-center"),
            { color: CONFIG.text },
          ]}
        >
          {userData.data.email}
        ...
        <View
          style={tailwind(
            "flex px-3 flex-row justify-between my-4 items-center w-full",
          )}
        >
      ...
      <View
        style={tailwind(
          "flex px-3 h-20 flex-row justify-around items-center w-full absolute bottom-7 bg-white",
        )}
      >
        <NiceButton type="danger" cb={logout}>
        ...
        {userData.data.email.split("@")[1] == "pausd.us" ? (
          <Barcode
            ...
            value={
              useFake
                ? "95051245"
                : "950" + userData.data.email.split("@")[0].substring(2)
            }
          ...
          <Text
            style={[tailwind("text-2xl text-center"), { color: CONFIG.text }]}
          >
            You must have a{" "}
            <Text style={{ color: CONFIG.green }}>PAUSD student email </Text>
            to use this feature.
          </Text>
        )}
      </View>
