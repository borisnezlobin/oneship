...
function ScheduleItem({ period, start, end, fixedHeight = false }) {
  ...
  return (
    <View
      style={[
        tailwind("w-full px-4 py-2"),
        {
          ...
          top: fixedHeight
            ? 0
            : ((period.start - start) / (end - start)) * screenHeight,
    ...
    >
      <Text
        style={[
          tailwind("font-bold text-lg w-full text-left"),
          { color: CONFIG.green },
        ]}
      >
        {userData && Object.keys(userData.data).includes(period.name)
          ? userData.data[period.name].customName
          : period.name}
      </Text>
      <Text>
      ...
      {isCurrent && fixedHeight && (
        <Text>Ending in {getTime(now, period.end)}</Text>
      )}
    </View>
  );
}
